...
- A user-built Slurm 25.05 RPM repository is available (see [Build Slurm Repo]())
- An NFS server is available with sufficient storage (minimum 50 GB for Slurm, 30 GB additional for CUDA if GPU nodes are used). See [Storage Requirements](../../Reference/ClusterRequirements/disk_space.md)
- For Slurm-only deployments (no service K8s), set `idrac_telemetry_support` to `false` in `telemetry_config.yml`
...
For detailed mount configuration including NFS options, VAST RDMA profiles, and functional group targeting, see [Configure Mounts](../Configure/configure_storage.md).
...
!!! tip
    ...
    [Configure InfiniBand](../Configure/configure_infiniband.md).
...
`/opt/omnia/discovery/output/project_default/discovery/` and set the `pxe_mapping_file_path`
variable in `provision_config.yml` to point to it.

```text title="File: /opt/omnia/discovery/output/project_default/discovery/bmc_pxe_mapping_file.csv"
...
[Discover Nodes Using OME](../Setup/../discovery/discover_nodes.md){target="_blank"}.
...
`/opt/omnia/discovery/output/project_default/discovery/`. Verify and edit the file as needed.
...
Edit [`local_repo_config.yml`](../../Reference/Configuration/repo_manager_config.md) and add
your Slurm RPM repository URL under `user_repo_url_x86_64` (and
`user_repo_url_aarch64` for ARM nodes):

```yaml title="File: /opt/omnia/repo_manager/input/project_default/local_repo_config.yml"
...
```yaml title="File: /opt/omnia/orchestrator/input/project_default/telemetry_config.yml"
telemetry_sources:
  dcgm:
...
!!! note
    ...
    For more information related to DCGM, see [DCGM](../Configure/slurm_with_gpu.md#dcgm).
...
[Configure Slurm](../Configure/configure_slurm.md).
...
For details, see
[Prepare OIM](../main/setup_oim.md){target="_blank"}.
...
[Verify OIM Services](../main/verify_oim_services.md){target="_blank"}.
...
For details, see
[Create Local Repos](../repo_manager/configure_repos.md){target="_blank"}.
...
For details, see
[Build Cluster Images](../image_build_manager/build_images.md){target="_blank"}.
...
[Prepare aarch64 Node](../utils/prepare_aarch64_node.md){target="_blank"}
...
For more information, see the [PXE Boot playbook](../Configure/configure_pxe_boot.md).
...
nodes. SSH from `OIM host` into each node using its hostname from the
PXE mapping file (`HOSTNAME` column):

```bash title="Run on: OIM host container (example for 2 nodes)"
...
- [Slurm with GPU](../Configure/slurm_with_gpu.md) -- Configure GPU support for Slurm nodes
- [NVIDIA HPC SDK Setup](../Configure/setup_nvhpc_sdk.md) -- Install NVIDIA HPC SDK on compiler and compute nodes
- [Add Slurm Nodes](../orchestrator/add_nodes.md) -- Add more compute nodes to the cluster
- [Config Backup](../utils/backup_slurm_config.md) -- Back up Slurm configuration
- [Run HPC Benchmarks](../Configure/run_hpc_benchmarks.md) -- Validate cluster performance
