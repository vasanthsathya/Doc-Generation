...
* Configure the proxy using the ``omnia_auth`` container. After deploying the ``omnia_auth`` container, perform the steps provided in :doc:`Configure OpenLDAP as a Proxy Server <OmniaCluster/OpenLDAP_proxy>`.
