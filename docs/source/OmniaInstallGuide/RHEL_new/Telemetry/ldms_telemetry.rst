...
5. 6. Ensure the ``ldms.json`` file contains the following entries for both ``x86_64`` and ``aarch64`` architectures.
...
6. In the ``local_repo_config.yml``, ensure the user repo paths defined for ``user_repo_url_x86_64`` and ``user_repo_url_aarch64`` contains the LDMS producer RPM for both ``x86_64`` and ``aarch64`` architectures. 
