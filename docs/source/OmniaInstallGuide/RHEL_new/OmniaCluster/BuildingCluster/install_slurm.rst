...
5. After successfully executing the ``discovery.yml`` playbook, PXE boot the nodes in the following sequence:
   ...
   1. Boot the ``service_kube_control_plane`` node first, before booting any other nodes in the cluster.
   2. Once the ``service_kube_control_plane`` node is up and running, you may simultaneously boot the ``service_kube_node``, ``login node``, and ``login_compiler_node`` nodes.
