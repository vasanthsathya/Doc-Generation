...
2. In the event of any modification to the ``functional_group_config.yml`` or ``software_config.json``, run the ``local_repo.yml`` playbook, and then run the ``build_image_x86_64.yml`` or ``build_image_aarch64.yml`` to build the new images. For more information, see see :doc:`RunningLocalRepo`.
3. After the images are created, run the ``discovery.yml`` playbook. For more information see :doc:`installprovisiontool`.
