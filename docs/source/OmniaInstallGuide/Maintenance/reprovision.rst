...
If you deploy the service Kubernetes cluster freshly, consider the following regarding ``server_share_path`` and ``client_share_path`` in your ``storage_config.yml``:

•	If you want to reuse the same ``server_share_path`` and ``client_share_path``:
...
•	If you want to use the new ``server_share_path`` and ``client_share_path``:
