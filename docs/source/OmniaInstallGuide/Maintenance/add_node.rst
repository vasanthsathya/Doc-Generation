...
2. Run the ``local_repo.yml`` playbook if there are any update made to the ``software_config.json``. For more information, see :doc:`../RHEL_new/CreateLocalRepo/RunningLocalRepo.rst`.
3. Run the ``build_image_x86_64.yml`` or ``build_image_aarch64.yml`` to build new images. For more information, :doc:`../RHEL_new/build_images.rst`.
