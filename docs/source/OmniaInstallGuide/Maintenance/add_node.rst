...
2. Run the ``local_repo.yml`` playbook if there are any update in the ``software_config.json``. For more information, see :doc:`RunningLocalRepo`.
3. Run the ``build_image_x86_64.yml`` or ``build_image_aarch64.yml`` to build the new images. For more information, see <build new images>
4. After the images are created, run the discovery.yml playbook. For more information, see :doc:`build_images`.
