...
.. note:: After running the ``oim_cleanup.yml`` playbook, ensure to reboot the OIM node. OIM cleanup does not delete the content of the ``server_share_path`` corresponding NFS shares of the service kubernetes cluster and slurm cluster.
