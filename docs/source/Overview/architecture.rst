...
.. image:: ../assets/images/omnia-slurm.png
   :alt: Omnia Slurm Stack


The Slurm cluster handles all HPC and AI compute workloads. Omnia deploys and
configures `Slurm <https://slurm.schedmd.com/>`_ as the job scheduler across
designated nodes.
...
.. note::
   ...
   machine. See `Composable Roles <composable_roles.md>`_ for details on how roles compose.
...
.. image:: ../assets/images/omnia-k8s.png
   :alt: Omnia Kubernetes Stack
...
.. list-table::
   ...
   * - Component
     - Purpose
   * - **MetalLB**
     - Bare-metal load balancer that assigns external IPs to Kubernetes ``LoadBalancer`` services without a cloud provider.
   * - **NFS CSI driver**
...
#. **Provisioning flow** -- The OIM discovers bare-metal nodes via BMC/iDRAC,
   ...
   or Kubernetes cluster based on the `Composable Roles <composable_roles.md>`_ mapping file.
...
#. **Telemetry** -- Metrics from all clusters flow into the
   `Telemetry Architecture <telemetry_architecture.md>`_ pipeline, which may run on the Kubernetes
...
.. note::
   ...
   - `Components <components.md>`_ -- Deep dive into each software component.
   - `Composable Roles <composable_roles.md>`_ -- How servers are assigned to clusters and roles.
   - `Network Topologies <network_topologies.md>`_ -- Network design options that connect the three
     clusters.
