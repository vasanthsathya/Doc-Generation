...
- The `Deploy Omnia Core <deploy_omnia_core.md>`_ procedure is complete and ``omnia_core`` is
  running.
...
.. list-table::
   ...
   * - ``FUNCTIONAL_GROUP_NAME``
     - Yes
     - Node role. Valid values: ``slurm_control_node``, ``slurm_node``, ``login_node``, ``kube_control_plane``, ``kube_node``, ``auth_server``
   * - ``GROUP_NAME``
     - Yes
     - Logical cluster grouping (e.g., ``slurm_cluster``, ``k8s_cluster``)
   * - ``SERVICE_TAG``
     - Yes
     - Dell service tag of the server (7-character alphanumeric string)
   * - ``PARENT_SERVICE_TAG``
     - Yes
     - Service tag of the parent chassis for blade servers. Leave empty for rack servers.
   * - ``HOSTNAME``
     - Yes
     - Custom hostname. Leave empty to let Omnia auto-generate from the service tag.
   * - ``ADMIN_MAC``
     - Yes
     - MAC address of the PXE-capable NIC on the admin network
   * - ``ADMIN_IP``
     - Yes
   ...
   * - ``BMC_IP``
     - Yes
     - Static IP address to assign on the BMC network
...
- `Configure Inputs <configure_inputs.md>`_ -- Configure software and cluster input files.
- `Configure Credentials <configure_credentials.md>`_ -- Set up encrypted provisioning credentials.
- `Discover Nodes <discover_nodes.md>`_ -- Run the discovery playbook using this mapping file.
