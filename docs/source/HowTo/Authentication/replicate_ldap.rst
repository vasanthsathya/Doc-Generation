...
Set up LDAP replication between the primary ``omnia_auth`` OpenLDAP server and
a secondary replica for redundancy and fault tolerance.
...
- The `Setup Openldap <setup_openldap.md>`_ procedure is complete (primary OpenLDAP is
...
.. code-block:: bash title="Run on: omnia_auth container (primary)"
...
.. code-block:: bash title="Run on: replica LDAP server"
...
- `Deploy External Ldap <deploy_external_ldap.md>`_ -- Deploy a fully external LDAP setup.
- `Setup Slurm <../Slurm/setup_slurm.md>`_ -- Slurm users authenticate via the
  replicated LDAP.
...
**"Invalid credentials" during replication**
   Verify the replicator account password matches between the provider
