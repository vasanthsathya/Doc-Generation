...
- The :doc:`Configure Inputs <configure_inputs>` procedure is complete (``network_spec.yml`` and
  ``provision_config.yml`` are configured).
- The :doc:`Configure Credentials <configure_credentials>` procedure is complete (encrypted credentials
  file exists).
...
- :doc:`Verify Oim Services <verify_oim_services>` -- Detailed verification of all OIM services.
- :doc:`Create Local Repos <create_local_repos>` -- Sync RPM repositories via Pulp.
- :doc:`Build Cluster Images <build_cluster_images>` -- Build OS images for cluster nodes.
