 ...
 * Path A: Slurm Quickstart [ Path A: Slurm Quickstart ](slurm_quickstart.html) Table of contents 
...
This tutorial assumes you have completed every item on the [Prerequisites Checklist](prerequisites_checklist.html). If you have not, stop here and finish that first.
...
`local_repo.yml` can take **30--60 minutes** depending on internet bandwidth. It downloads several GB of packages. Ensure the OIM has sufficient disk space (see [Prerequisites Checklist](prerequisites_checklist.html)) and a stable internet connection.
...
**Enable telemetry and monitoring** Follow [Full Deployment](full_deployment.html) (Path B) starting from the telemetry sections to add Grafana dashboards and iDRAC metrics collection.

**Configure authentication (LDAP/FreeIPA)** Edit `security_config.yml` to enable FreeIPA or LDAP, then run `auth.yml`. See the [Full Deployment](full_deployment.html) guide for details.
 ...
 * [Index](../Overview/index.html) \-- Architecture and component overview
 * [Full Deployment](full_deployment.html) \-- Add K8s and telemetry to this cluster
 * [Prerequisites Checklist](prerequisites_checklist.html) \-- Return to the master checklist

Back to top [ Previous Prerequisites Checklist ](prerequisites_checklist.html) [ Next Path B: Full Deployment ](full_deployment.html)
